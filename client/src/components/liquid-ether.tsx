...
`;
        ...
        super({
          material: {
            ...
            uniforms: {
              ...
              divergence: { value: simProps.div.texture },
              px: { value: simProps.cellScale }
            }
          },
          output: simProps.dst,
          output0: simProps.dst,
          output1: simProps.dst_
        });
        this.init();
      }
      update({ iterations }) {
      ...
      constructor(simProps) {
        super({
          material: {
            ...
            uniforms: {
              boundarySpace: { value: simProps.boundarySpace },
              pressure: { value: simProps.press.texture },
              velocity: { value: simProps.vel.texture },
              px: { value: simProps.cellScale },
              dt: { value: simProps.dt }
            }
    ...
    class Output extends ShaderPass {
      constructor(simProps) {
        ...
        this.init();
        this.simulation = simProps.simulation;
      }
      update() {
        this.uniforms.velocity.value = this.simulation.fbos.vel_1.texture;
        super.update();
      }
    }
    ...
    class Simulation {
      ...
      createShaderPass() {
        ...
        this.pressure = new Pressure({
          ...
          boundarySpace: this.cellScale,
          dt: this.options.dt,
          press: this.fbos.pressure_1,
          vel: this.fbos.vel_1,
          dst: this.fbos.vel_0
        });
      }
      resize() {
        const cellsX = Math.round(Common.width * this.options.resolution);
        const cellsY = Math.round(Common.height * this.options.resolution);
        Common.fboWidth = cellsX;
        Common.fboHeight = cellsY;
        this.cellScale.set(1.0 / cellsX, 1.0 / cellsY);
        this.fboSize.set(cellsX, cellsY);
      }
      update() {
        ...
        const fbo = this.poisson.update({ iterations: this.options.iterations_poisson });
        this.fbos.pressure_1 = fbo;
        this.pressure.update({
          press: this.fbos.pressure_1,
          vel: this.fbos.vel_1
        });
        const t = this.fbos.vel_0;
    ...
    class WebGLManager {
      ...
      start() {
        ...
        if (!this.output) {
          ...
          this.output = new Output({ src: sim.fbos.vel_0, simulation: sim });
        }
        this.loop();
      }
      pause() {
      ...
      resize() {
        Common.resize();
        if (this.output?.simulation) this.output.simulation.resize();
      }
      loop() {
        if (this.isPaused) return;
