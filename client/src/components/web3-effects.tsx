...
export function Web3Effects() {
  ...
  useEffect(() => {
    ...
    if (!canvas) {
      console.log("Web3Effects: No canvas found");
      return;
    }

    const ctx = canvas.getContext("2d");
    if (!ctx) {
      console.log("Web3Effects: No canvas context");
      return;
    }

    console.log("Web3Effects: Initializing...");

    // Set canvas size
    const resizeCanvas = () => {
      canvas.width = window.innerWidth;
      canvas.height = window.innerHeight;
      console.log(`Web3Effects: Canvas resized to ${canvas.width}x${canvas.height}`);
    };
    resizeCanvas();
    ...
    const initParticles = () => {
      ...
      console.log(`Web3Effects: Initialized ${particlesRef.current.length} particles`);
    };
    initParticles();
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Keep last 15 trail points
      if (mouseTrailRef.current.length > 15) {
        mouseTrailRef.current = mouseTrailRef.current.slice(-15);
      }
    };
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      console.log(`Web3Effects: Click at (${e.clientX}, ${e.clientY})`);
    };

    window.addEventListener("mousemove", handleMouseMove);
    window.addEventListener("click", handleClick);

    console.log("Web3Effects: Starting animation loop");
    ...
    const animate = () => {
      ...
      mouseTrail.forEach((point, index) => {
        ...
        const size = Math.max(0.1, 20 * (1 - age / 1000));

        if (size > 0.1 && opacity > 0) {
          ctx.beginPath();
      ...
      mouseTrailRef.current = mouseTrail.filter(p => now - p.timestamp < 1000);
      ...
      clickEffects.forEach(effect => {
        ...
        if (progress < 1) {
          ...
          for (let i = 0; i < 8; i++) {
            ...
            const particleSize = Math.max(0.1, 3 * (1 - progress));
    ...
    animate();
    console.log("Web3Effects: Animation loop started");

    return () => {
      console.log("Web3Effects: Cleaning up");
